...
//contract('Business', function(accounts){
    ...
    it("should allow business owner to invoice a customer", function(){
        return Business.at(business_contract_address1).then(function(instance){
                return instance.processDocument(customer_address1, -1*invoice_amount1, ipfsHash, due_date, {'from' : business_address1});
        }).then(function(receipt) {
            assert.equal(receipt.logs.length, 1, "an event should have been triggered");
            assert.equal(receipt.logs[0].event, "documentProcessed", "event should be documentProcessed");
    ...
    it("should update the customer balance", function(){
        return Business.at(business_contract_address1).then(function(instance){
            ...
            assert.equal(details[2], true, "customer must be active");    
        });
    });
